...
class Grid:
    def __init__(self):
        self.grid = [[Cell() for i in range(NUM_CELLS)] for j in range(NUM_CELLS)]
        ...
        for i in range(NUM_CELLS):
            for j in range(NUM_CELLS):
                ...
                if self.grid[i][j].checkRevealed(): # Darker
                    ...
                    self.grid[i][j].showImage(screen, i * CELL_SIZE, j * CELL_SIZE)
                else: # Lighter
                    pygame.draw.rect(screen, LIGHT_GREY, rect, 0)
                    if self.grid[i][j].checkFlag():
                        self.grid[i][j].showFlag(screen, i * CELL_SIZE, j * CELL_SIZE)
